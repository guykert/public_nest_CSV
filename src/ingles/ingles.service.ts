...
export class InglesService {
  ...
  constructor(
    @InjectModel( Ingle.name )
    private readonly ingleModel: Model<Ingle>,
    @InjectModel( Ingle.name )
    private readonly ingleModelPag: PaginateModel<Ingle>,
  ...
  async create(createIngleDto: CreateIngleDto) {
    try {
      const user = await  this.ingleModel.create({...createIngleDto,createdAt: new Date()});
  ...
  async findAll(paginationDto: PaginationDto) {
    try {
      ...
      const data = await this.ingleModelPag.paginate(query, options);
  ...
  async findOne(id: string) {
    ...
    //   data = await this.ingleModel.findById(id,{activo:0,createdAt:0,updatedAt:0});
    ...
    return  await this.ingleModel.findById(id,{activo:0,createdAt:0,updatedAt:0});
  }

  async update(id: string, updateIngleDto: UpdateIngleDto) {
    // try {
  
    //   return await this.ingleModel.findOneAndUpdate({ _id: id}, {...updateIngleDto,updatedAt: new Date()}, { new: true });
  ...
  async remove(id: string) {
    ...
    //   await this.ingleModel.findByIdAndDelete(id);
