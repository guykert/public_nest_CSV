...
describe('InglesService', () => {
  let service: PruebaTipoService;
  let PruebaTipoModel: Model<PruebaTipo>;
  ...
  const mockModel ={
      ...
      update: jest
      .fn()
      .mockImplementation((id: 'a uuid', updateDto: UpdatePruebaTipoDto) =>
  ...
  beforeEach(async () => {
    const module: TestingModule = await Test.createTestingModule({
      ...
      providers: [
        ...
        {
          provide: getModelToken(PruebaTipo.name),
          useValue: mockModel
        },
    ...
    service = module.get<PruebaTipoService>(PruebaTipoService);

    

    PruebaTipoModel = module.get<Model<PruebaTipo>>(getModelToken(PruebaTipo.name));
  ...
  describe('insertOne', () => {
    it('should successfully insert a cat', () => {
      ...
      expect(PruebaTipoModel.create).toBeCalledTimes(1);
      expect(PruebaTipoModel.create).toBeCalledWith({
        ...DATAMOCK.One,
  ...
  describe('updateOne', () => {
    it('should call the update method', async () => {
      ...
      const updateDto: UpdatePruebaTipoDto = DATAMOCK.One;
      const solicitudId = "a uuid";

      const data = await service.update(solicitudId, updateDto);
      expect(data).toEqual(DATAMOCK.OneResponse);
      expect(PruebaTipoModel.findOneAndUpdate).toBeCalledTimes(1);
      expect(PruebaTipoModel.findOneAndUpdate).toBeCalledWith(
        { _id: 'a uuid' },
  ...
  describe('deleteOne', () => {
    ...
    it('should return {deleted: false, message: err.message}', async () => {
      const repoSpy = jest
        .spyOn(PruebaTipoModel, 'findByIdAndDelete')
