...
describe('InglesService', () => {
  ...
  let TipoEmpresaModel: Model<TipoEmpresa>;
  ...
  beforeEach(async () => {
    const module: TestingModule = await Test.createTestingModule({
      ...
      providers: [
        ...
        {
          provide: getModelToken(TipoEmpresa.name),
          useValue: mockModel
        },
    ...
    TipoEmpresaModel = module.get<Model<TipoEmpresa>>(getModelToken(TipoEmpresa.name));
  ...
  describe('insertOne', () => {
    it('should successfully insert a cat', () => {
      ...
      expect(TipoEmpresaModel.create).toBeCalledTimes(1);
      expect(TipoEmpresaModel.create).toBeCalledWith({
        ...DATAMOCK.One,
  ...
  describe('updateOne', () => {
    it('should call the update method', async () => {
      ...
      expect(TipoEmpresaModel.findOneAndUpdate).toBeCalledTimes(1);
      expect(TipoEmpresaModel.findOneAndUpdate).toBeCalledWith(
        { _id: 'a uuid' },
  ...
  describe('deleteOne', () => {
    ...
    it('should return {deleted: false, message: err.message}', async () => {
      const repoSpy = jest
        .spyOn(TipoEmpresaModel, 'findByIdAndDelete')
