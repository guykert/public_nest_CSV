...
describe('InglesService', () => {
  let service: MenuTipoService;
  let MenuTipoModel: Model<MenuTipo>;
  ...
  const mockModel ={
      ...
      update: jest
      .fn()
      .mockImplementation((id: 'a uuid', updateDto: UpdateMenuTipoDto) =>
  ...
  beforeEach(async () => {
    const module: TestingModule = await Test.createTestingModule({
      ...
      providers: [
        ...
        {
          provide: getModelToken(MenuTipo.name),
          useValue: mockModel
        },
    ...
    service = module.get<MenuTipoService>(MenuTipoService);

    

    MenuTipoModel = module.get<Model<MenuTipo>>(getModelToken(MenuTipo.name));
  ...
  describe('insertOne', () => {
    it('should successfully insert a cat', () => {
      ...
      expect(MenuTipoModel.create).toBeCalledTimes(1);
      expect(MenuTipoModel.create).toBeCalledWith({
        ...DATAMOCK.One,
  ...
  describe('updateOne', () => {
    it('should call the update method', async () => {
      ...
      const updateDto: UpdateMenuTipoDto = DATAMOCK.One;
      const solicitudId = "a uuid";

      const data = await service.update(solicitudId, updateDto);
      expect(data).toEqual(DATAMOCK.OneResponse);
      expect(MenuTipoModel.findOneAndUpdate).toBeCalledTimes(1);
      expect(MenuTipoModel.findOneAndUpdate).toBeCalledWith(
        { _id: 'a uuid' },
  ...
  describe('deleteOne', () => {
    ...
    it('should return {deleted: false, message: err.message}', async () => {
      const repoSpy = jest
        .spyOn(MenuTipoModel, 'findByIdAndDelete')
