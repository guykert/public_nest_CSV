...
describe('InglesService', () => {
  ...
  const mockModel ={
      ...
      update: jest
      .fn()
      .mockImplementation((id: 'a uuid', updateDto: UpdateProyectoDto) =>
  ...
  beforeEach(async () => {
    ...
    ProyectoModel = module.get<Model<Proyecto>>(getModelToken(Proyecto.name));
  ...
  describe('insertOne', () => {
    it('should successfully insert a cat', () => {
      ...
      expect(ProyectoModel.create).toBeCalledTimes(1);
      expect(ProyectoModel.create).toBeCalledWith({
        ...DATAMOCK.One,
  ...
  describe('updateOne', () => {
    it('should call the update method', async () => {
      ...
      const updateDto: UpdateProyectoDto = DATAMOCK.One;
      const solicitudId = "a uuid";

      const data = await service.update(solicitudId, updateDto);
      expect(data).toEqual(DATAMOCK.OneResponse);
      expect(ProyectoModel.findOneAndUpdate).toBeCalledTimes(1);
      expect(ProyectoModel.findOneAndUpdate).toBeCalledWith(
        { _id: 'a uuid' },
  ...
  describe('deleteOne', () => {
    ...
    it('should return {deleted: false, message: err.message}', async () => {
      const repoSpy = jest
        .spyOn(ProyectoModel, 'findByIdAndDelete')
