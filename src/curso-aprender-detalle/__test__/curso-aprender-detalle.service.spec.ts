...
describe('CursoAprenderService', () => {
  let service: CursoAprenderDetalleService;
  let CursoAprenderDetalleModel: Model<CursoAprenderDetalle>;
  ...
  const mockModel ={
      ...
      update: jest
      .fn()
      .mockImplementation((id: 'a uuid', updateDto: UpdateCursoAprenderDetalleDto) =>
  ...
  beforeEach(async () => {
    const module: TestingModule = await Test.createTestingModule({
      ...
      providers: [
        CursoAprenderDetalleService,
        
        {
          provide: getModelToken(CursoAprenderDetalle.name),
          useValue: mockModel
        },
    ...
    service = module.get<CursoAprenderDetalleService>(CursoAprenderDetalleService);

    

    CursoAprenderDetalleModel = module.get<Model<CursoAprenderDetalle>>(getModelToken(CursoAprenderDetalle.name));
  ...
  describe('insertOne', () => {
    it('should successfully insert a cat', () => {
      ...
      expect(CursoAprenderDetalleModel.create).toBeCalledTimes(1);
      expect(CursoAprenderDetalleModel.create).toBeCalledWith({
        ...DATAMOCK.One,
  ...
  describe('updateOne', () => {
    it('should call the update method', async () => {
      ...
      const updateDto: UpdateCursoAprenderDetalleDto = DATAMOCK.One;
      const solicitudId = "a uuid";

      const data = await service.update(solicitudId, updateDto);
      expect(data).toEqual(DATAMOCK.OneResponse);
      expect(CursoAprenderDetalleModel.findOneAndUpdate).toBeCalledTimes(1);
      expect(CursoAprenderDetalleModel.findOneAndUpdate).toBeCalledWith(
        { _id: 'a uuid' },
  ...
  describe('deleteOne', () => {
    ...
    it('should return {deleted: false, message: err.message}', async () => {
      const repoSpy = jest
        .spyOn(CursoAprenderDetalleModel, 'findByIdAndDelete')
