...
describe('InglesService', () => {
  let service: PruebaEjeTematicoService;
  let PruebaEjeTematicoModel: Model<PruebaEjeTematico>;
  ...
  const mockModel ={
      ...
      update: jest
      .fn()
      .mockImplementation((id: 'a uuid', updateDto: UpdatePruebaEjeTematicoDto) =>
  ...
  beforeEach(async () => {
    const module: TestingModule = await Test.createTestingModule({
      ...
      providers: [
        PruebaEjeTematicoService,
        
        {
          provide: getModelToken(PruebaEjeTematico.name),
          useValue: mockModel
        },
    ...
    service = module.get<PruebaEjeTematicoService>(PruebaEjeTematicoService);

    

    PruebaEjeTematicoModel = module.get<Model<PruebaEjeTematico>>(getModelToken(PruebaEjeTematico.name));
  ...
  describe('insertOne', () => {
    it('should successfully insert a cat', () => {
      ...
      expect(PruebaEjeTematicoModel.create).toBeCalledTimes(1);
      expect(PruebaEjeTematicoModel.create).toBeCalledWith({
        ...DATAMOCK.One,
  ...
  describe('updateOne', () => {
    it('should call the update method', async () => {
      ...
      const updateDto: UpdatePruebaEjeTematicoDto = DATAMOCK.One;
      const solicitudId = "a uuid";

      const data = await service.update(solicitudId, updateDto);
      expect(data).toEqual(DATAMOCK.OneResponse);
      expect(PruebaEjeTematicoModel.findOneAndUpdate).toBeCalledTimes(1);
      expect(PruebaEjeTematicoModel.findOneAndUpdate).toBeCalledWith(
        { _id: 'a uuid' },
  ...
  describe('deleteOne', () => {
    ...
    it('should return {deleted: false, message: err.message}', async () => {
      const repoSpy = jest
        .spyOn(PruebaEjeTematicoModel, 'findByIdAndDelete')
