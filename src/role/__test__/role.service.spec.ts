...
describe('InglesService', () => {
  ...
  const mockModel ={
      ...
      update: jest
      .fn()
      .mockImplementation((id: 'a uuid', updateDto: UpdateRoleDto) =>
  ...
  describe('insertOne', () => {
    it('should successfully insert a cat', () => {
      ...
      expect(RoleModel.create).toBeCalledTimes(1);
      expect(RoleModel.create).toBeCalledWith({
        ...DATAMOCK.One,
  ...
  describe('updateOne', () => {
    it('should call the update method', async () => {
      ...
      const updateDto: UpdateRoleDto = DATAMOCK.One;
      const solicitudId = "a uuid";

      const data = await service.update(solicitudId, updateDto);
      expect(data).toEqual(DATAMOCK.OneResponse);
      expect(RoleModel.findOneAndUpdate).toBeCalledTimes(1);
      expect(RoleModel.findOneAndUpdate).toBeCalledWith(
        { _id: 'a uuid' },
  ...
  describe('deleteOne', () => {
    ...
    it('should return {deleted: false, message: err.message}', async () => {
      const repoSpy = jest
        .spyOn(RoleModel, 'findByIdAndDelete')
