...
describe('InglesService', () => {
  ...
  let PruebaAlumnoModel: Model<PruebaAlumno>;
  ...
  beforeEach(async () => {
    const module: TestingModule = await Test.createTestingModule({
      ...
      providers: [
        ...
        {
          provide: getModelToken(PruebaAlumno.name),
          useValue: mockModel
        },
    ...
    PruebaAlumnoModel = module.get<Model<PruebaAlumno>>(getModelToken(PruebaAlumno.name));
  ...
  describe('insertOne', () => {
    it('should successfully insert a cat', () => {
      ...
      expect(PruebaAlumnoModel.create).toBeCalledTimes(1);
      expect(PruebaAlumnoModel.create).toBeCalledWith({
        ...DATAMOCK.One,
  ...
  describe('updateOne', () => {
    it('should call the update method', async () => {
      ...
      expect(PruebaAlumnoModel.findOneAndUpdate).toBeCalledTimes(1);
      expect(PruebaAlumnoModel.findOneAndUpdate).toBeCalledWith(
        { _id: 'a uuid' },
  ...
  describe('deleteOne', () => {
    ...
    it('should return {deleted: false, message: err.message}', async () => {
      const repoSpy = jest
        .spyOn(PruebaAlumnoModel, 'findByIdAndDelete')
