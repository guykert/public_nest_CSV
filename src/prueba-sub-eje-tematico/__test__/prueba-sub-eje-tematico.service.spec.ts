...
describe('InglesService', () => {
  ...
  let PruebaSubEjeTematicoModel: Model<PruebaSubEjeTematico>;
  ...
  beforeEach(async () => {
    const module: TestingModule = await Test.createTestingModule({
      ...
      providers: [
        PruebaSubEjeTematicoService,
        
        {
          provide: getModelToken(PruebaSubEjeTematico.name),
          useValue: mockModel
        },
    ...
    PruebaSubEjeTematicoModel = module.get<Model<PruebaSubEjeTematico>>(getModelToken(PruebaSubEjeTematico.name));
  ...
  describe('insertOne', () => {
    it('should successfully insert a cat', () => {
      ...
      expect(PruebaSubEjeTematicoModel.create).toBeCalledTimes(1);
      expect(PruebaSubEjeTematicoModel.create).toBeCalledWith({
        ...DATAMOCK.One,
  ...
  describe('updateOne', () => {
    it('should call the update method', async () => {
      ...
      expect(PruebaSubEjeTematicoModel.findOneAndUpdate).toBeCalledTimes(1);
      expect(PruebaSubEjeTematicoModel.findOneAndUpdate).toBeCalledWith(
        { _id: 'a uuid' },
  ...
  describe('deleteOne', () => {
    ...
    it('should return {deleted: false, message: err.message}', async () => {
      const repoSpy = jest
        .spyOn(PruebaSubEjeTematicoModel, 'findByIdAndDelete')
