...
export class PaisService {
  ...
  constructor(
    ...
    @InjectModel( Pais.name )
    private readonly paisModelPag: PaginateModel<Pais>,
  ...
  async create(createPaiDto: CreatePaiDto) {
    try {
      const user = await  this.paisModel.create({...createPaiDto,createdAt: new Date()});
  ...
  async findAll(paginationDto: PaginationDto) {
    try {
      ...
      const data = await this.paisModelPag.paginate(query, options);
  ...
  async findOne(id: string) {
    ...
    //   data = await this.paisModel.findById(id,{activo:0,createdAt:0,updatedAt:0});
    ...
    return await this.paisModel.findById(id,{activo:0,createdAt:0,updatedAt:0});
  ...
  async update(id: string, updatePaiDto: UpdatePaiDto) {
    ...
    //   return await this.paisModel.findOneAndUpdate({ _id: id}, {...updatePaiDto,updatedAt: new Date()}, { new: true });
  ...
  async remove(id: string) {
    ...
    //   await this.paisModel.findByIdAndDelete(id);
