...
describe('InglesService', () => {
  ...
  const mockModel ={
      ...
      update: jest
      .fn()
      .mockImplementation((id: 'a uuid', updateDto: UpdatePaiDto) =>
  ...
  beforeEach(async () => {
    const module: TestingModule = await Test.createTestingModule({
      ...
      providers: [
        ...
        {
          provide: getModelToken(Pais.name),
          useValue: mockModel
        },
    ...
    PaisModel = module.get<Model<Pais>>(getModelToken(Pais.name));
  ...
  describe('insertOne', () => {
    it('should successfully insert a cat', () => {
      ...
      expect(PaisModel.create).toBeCalledTimes(1);
      expect(PaisModel.create).toBeCalledWith({
        ...DATAMOCK.One,
  ...
  describe('updateOne', () => {
    it('should call the update method', async () => {
      ...
      const updateDto: UpdatePaiDto = DATAMOCK.One;
      const solicitudId = "a uuid";

      const data = await service.update(solicitudId, updateDto);
      expect(data).toEqual(DATAMOCK.OneResponse);
      expect(PaisModel.findOneAndUpdate).toBeCalledTimes(1);
      expect(PaisModel.findOneAndUpdate).toBeCalledWith(
        { _id: 'a uuid' },
  ...
  describe('deleteOne', () => {
    ...
    it('should return {deleted: false, message: err.message}', async () => {
      const repoSpy = jest
        .spyOn(PaisModel, 'findByIdAndDelete')
