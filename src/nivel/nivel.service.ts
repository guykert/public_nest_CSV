...
export class NivelService {
  ...
  constructor(
    ...
    @InjectModel( Nivel.name )
    private readonly nivelModelPag: PaginateModel<Nivel>,
  ...
  async create(createNivelDto: CreateNivelDto) {
    try {
      const user = await  this.nivelModel.create({...createNivelDto,createdAt: new Date()});
  ...
  async findAll(paginationDto: PaginationDto) {
    try {
      ...
      const data = await this.nivelModelPag.paginate(query, options);
  ...
  async findOne(id: string) {
    ...
    //   data = await this.nivelModel.findById(id,{activo:0,createdAt:0,updatedAt:0});
    ...
    return await this.nivelModel.findById(id,{activo:0,createdAt:0,updatedAt:0});

  }

  async update(id: string, updateNivelDto: UpdateNivelDto) {
    // try {
  
    //   return await this.nivelModel.findOneAndUpdate({ _id: id}, {...updateNivelDto,updatedAt: new Date()}, { new: true });
