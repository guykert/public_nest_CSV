...
describe('InglesService', () => {
  ...
  beforeEach(async () => {
    const module: TestingModule = await Test.createTestingModule({
      ...
      providers: [
        ...
        {
          provide: getModelToken(Nivel.name),
          useValue: mockModel
        },
    ...
    NivelModel = module.get<Model<Nivel>>(getModelToken(Nivel.name));
  ...
  describe('insertOne', () => {
    it('should successfully insert a cat', () => {
      ...
      expect(NivelModel.create).toBeCalledTimes(1);
      expect(NivelModel.create).toBeCalledWith({
        ...DATAMOCK.One,
  ...
  describe('updateOne', () => {
    it('should call the update method', async () => {
      ...
      expect(NivelModel.findOneAndUpdate).toBeCalledTimes(1);
      expect(NivelModel.findOneAndUpdate).toBeCalledWith(
        { _id: 'a uuid' },
  ...
  describe('deleteOne', () => {
    ...
    it('should return {deleted: false, message: err.message}', async () => {
      const repoSpy = jest
        .spyOn(NivelModel, 'findByIdAndDelete')
