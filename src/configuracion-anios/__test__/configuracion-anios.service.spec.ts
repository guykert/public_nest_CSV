...
describe('InglesService', () => {
  let service: ConfiguracionAniosService;
  let ConfiguracionAnioModel: Model<ConfiguracionAnio>;
  ...
  const mockModel ={
      ...
      update: jest
      .fn()
      .mockImplementation((id: 'a uuid', updateDto: UpdateConfiguracionAnioDto) =>
  ...
  beforeEach(async () => {
    const module: TestingModule = await Test.createTestingModule({
      ...
      providers: [
        ConfiguracionAniosService,
        
        {
          provide: getModelToken(ConfiguracionAnio.name),
          useValue: mockModel
        },
    ...
    service = module.get<ConfiguracionAniosService>(ConfiguracionAniosService);

    

    ConfiguracionAnioModel = module.get<Model<ConfiguracionAnio>>(getModelToken(ConfiguracionAnio.name));
  ...
  describe('insertOne', () => {
    it('should successfully insert a cat', () => {
      ...
      expect(ConfiguracionAnioModel.create).toBeCalledTimes(1);
      expect(ConfiguracionAnioModel.create).toBeCalledWith({
        ...DATAMOCK.One,
  ...
  describe('updateOne', () => {
    it('should call the update method', async () => {
      ...
      const updateDto: UpdateConfiguracionAnioDto = DATAMOCK.One;
      const solicitudId = "a uuid";

      const data = await service.update(solicitudId, updateDto);
      expect(data).toEqual(DATAMOCK.OneResponse);
      expect(ConfiguracionAnioModel.findOneAndUpdate).toBeCalledTimes(1);
      expect(ConfiguracionAnioModel.findOneAndUpdate).toBeCalledWith(
        { _id: 'a uuid' },
  ...
  describe('deleteOne', () => {
    ...
    it('should return {deleted: false, message: err.message}', async () => {
      const repoSpy = jest
        .spyOn(ConfiguracionAnioModel, 'findByIdAndDelete')
