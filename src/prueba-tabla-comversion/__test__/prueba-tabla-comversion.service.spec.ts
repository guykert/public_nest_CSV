...
describe('InglesService', () => {
  let service: PruebaTablaComversionService;
  let PruebaTablaComversionModel: Model<PruebaTablaComversion>;
  ...
  const mockModel ={
      ...
      update: jest
      .fn()
      .mockImplementation((id: 'a uuid', updateDto: UpdatePruebaTablaComversionDto) =>
  ...
  beforeEach(async () => {
    const module: TestingModule = await Test.createTestingModule({
      ...
      providers: [
        PruebaTablaComversionService,
        
        {
          provide: getModelToken(PruebaTablaComversion.name),
          useValue: mockModel
        },
    ...
    service = module.get<PruebaTablaComversionService>(PruebaTablaComversionService);

    

    PruebaTablaComversionModel = module.get<Model<PruebaTablaComversion>>(getModelToken(PruebaTablaComversion.name));
  ...
  describe('insertOne', () => {
    it('should successfully insert a cat', () => {
      ...
      expect(PruebaTablaComversionModel.create).toBeCalledTimes(1);
      expect(PruebaTablaComversionModel.create).toBeCalledWith({
        ...DATAMOCK.One,
  ...
  describe('updateOne', () => {
    it('should call the update method', async () => {
      ...
      const updateDto: UpdatePruebaTablaComversionDto = DATAMOCK.One;
      const solicitudId = "a uuid";

      const data = await service.update(solicitudId, updateDto);
      expect(data).toEqual(DATAMOCK.OneResponse);
      expect(PruebaTablaComversionModel.findOneAndUpdate).toBeCalledTimes(1);
      expect(PruebaTablaComversionModel.findOneAndUpdate).toBeCalledWith(
        { _id: 'a uuid' },
  ...
  describe('deleteOne', () => {
    ...
    it('should return {deleted: false, message: err.message}', async () => {
      const repoSpy = jest
        .spyOn(PruebaTablaComversionModel, 'findByIdAndDelete')
