...
describe('InglesService', () => {
  ...
  let InglesDetalleModel: Model<InglesDetalle>;
  ...
  beforeEach(async () => {
    const module: TestingModule = await Test.createTestingModule({
      ...
      providers: [
        InglesDetalleService,
        
        {
          provide: getModelToken(InglesDetalle.name),
          useValue: mockModel
        },
    ...
    InglesDetalleModel = module.get<Model<InglesDetalle>>(getModelToken(InglesDetalle.name));
  ...
  describe('insertOne', () => {
    it('should successfully insert a cat', () => {
      ...
      expect(InglesDetalleModel.create).toBeCalledTimes(1);
      expect(InglesDetalleModel.create).toBeCalledWith({
        ...DATAMOCK.One,
  ...
  describe('updateOne', () => {
    it('should call the update method', async () => {
      ...
      expect(InglesDetalleModel.findOneAndUpdate).toBeCalledTimes(1);
      expect(InglesDetalleModel.findOneAndUpdate).toBeCalledWith(
        { _id: 'a uuid' },
  ...
  describe('deleteOne', () => {
    ...
    it('should return {deleted: false, message: err.message}', async () => {
      const repoSpy = jest
        .spyOn(InglesDetalleModel, 'findByIdAndDelete')
